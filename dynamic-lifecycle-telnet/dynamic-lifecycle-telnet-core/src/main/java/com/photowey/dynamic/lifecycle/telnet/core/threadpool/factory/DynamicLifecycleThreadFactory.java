...
/**
 * {@code DynamicLifecycleThreadFactory}
 *
 * @author photowey
 * @date 2021/12/05
 * @since 1.0.0
 */
public class DynamicLifecycleThreadFactory implements ThreadFactory {

    private static final AtomicLong THREAD_NUMBER = new AtomicLong(1);
    private static final ThreadGroup THREAD_GROUP = new ThreadGroup("dynamic");

    private final boolean daemon;
    private final String prefix;
    private final int priority;

    private DynamicLifecycleThreadFactory(final String prefix, final boolean daemon, final int priority) {
        this.prefix = prefix;
        this.daemon = daemon;
    ...
    public static ThreadFactory create(final String prefix, final boolean daemon, final int priority) {
        return new DynamicLifecycleThreadFactory(prefix, daemon, priority);
    }
